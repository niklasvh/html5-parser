...
function Parser(html, options) {
    ...
    this.tokens = [];
    this.fragmentCase = false;
}

Parser.prototype.parseHTMLFragment = function(context) {
    ...
    switch(context.tagName) {
        ...
        case "NOEMBED":
        case "NOFRAMES":
            this.tokenizer.setState(this.tokenizer.states.rawtext);
            break;
        case "SCRIPT":
            this.tokenizer.setState(this.tokenizer.states.script_data);
            break;
        case "NOSCRIPT":
            this.tokenizer.setState(this.tokenizer.states.rawtext);
            break;
        case "PLAINTEXT":
            ...
            break;
        default:
            this.tokenizer.setState(this.tokenizer.states.data);
    }

    var root = this.constructor.createElement("html");
    this.constructor._appendChild(root);
    this.openElements.push(root);
    this.context = context;
    this.resetInsertionMode();

    this.run();
    // TODO If the context element is a template element, push "in template" onto the stack of template insertion modes so that it is the new current template insertion mode.
...
Parser.prototype.run = function() {
    ...
    do {
        ...
        if (Array.isArray(result)) {
            result.forEach(this.addToken, this);
        }
    } while(result !== false);
...
Parser.prototype.adjustedCurrentNode = function() {
    if (this.openElements.length) {
        if (this.openElements.length === 1 && this.fragmentCase) {
            return this.context;
        }
...
Parser.prototype.initial = function(token) {
    if (this.isCharacterToken(token) && this.characterOneOf(token, characters.TAB, characters.LF, characters.FF, characters.CR, characters.SPACE)) {
        ...
        this.constructor._quirksMode = true;
