(function(exports) {
  ...
  Parser.prototype.insertBlock = function(name) {
    var block = this.constructor.createBlock(name);
    this.currentNode().childNodes.push(block);
    this.openElements.push(block);
    return block;
  };

  Parser.prototype.insertWithBlock = function(type, name) {
    var block = this.constructor.createWithBlock(type, name);
    this.currentNode().childNodes.push(block);
    this.openElements.push(block);
    return block;
  };

  Parser.prototype.closeBlock = function(name) {
    var currentNode = this.currentNode();
    if (currentNode.nodeType !== -2 && currentNode.nodeType !== -3) {
      throw new Error("Currently open node isn't a block, attempting to close #" + name);
    }
    if (currentNode._tagName !== name) {
      throw new Error("Currently open block (#" + currentNode._tagName + ") does not match closing tag (/" + name + ")");
    }
  ...
  Parser.prototype.in_body = function(token) {
    ...
    } else if (this.isBlock(token)) {
      this.insertBlock(token[1]);
    } else if (this.isWithBlock(token)) {
      this.insertWithBlock(token[1], token[2]);
    } else if (this.isCloseBlock(token)) {
      this.closeBlock(token[1]);
    }
  };
  ...
  Parser.prototype.isBlock = function(token) {
    return token[0] === tokenType.block;
  };

  Parser.prototype.isWithBlock = function(token) {
    return token[0] === tokenType.withBlock;
  };

  Parser.prototype.isCloseBlock = function(token) {
    return token[0] === tokenType.closeBlock;
  };
