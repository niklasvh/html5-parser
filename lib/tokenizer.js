...
(function(exports) {
    ...
    exports.tokenizerStates = states;

    exports.tokenizer = function(html, options) {
        options = options || {};
        var i = 0;
        ...
        var state = options.initialState || states.data;
        ...
        var tokens = [];
        var lastStartTag = options.lastStartTag || null;
        var characterBuffer = "";
        ...
        function isSurrogatePair(input, pos) {
            if (!input[pos + 1]) {
                return false;
            }
         ...
         * MIT license
         */
        function codePointAt(input, pos) {
            if (!input[pos + 1]) {
            ...
            switch (getCharacter(input)) {
                ...
                default:
                    ...
                    if (found.length === 0) {
                        ...
                        if ((input[position + c-1] === String.fromCharCode(characters.SEMICOLON)) || (inAttribute && input[position + c-1] !== String.fromCharCode(characters.SEMICOLON)) || input[position + c-1] === String.fromCharCode(characters.EQUALS_SIGN) || !alphanumericASCII.test(input[position]) || !input[position + c]) {
                            parseError();
                        }
                        return null;
                    }
        ...
        function data_state(character) {
            switch(character) {
                ...
                default:
                    ...
                    } else if(invalidCharacters(codePointAt(input, position - 1))) {
                        parseErrorWithFlush(characterBuffer.length === 0);
                    }

                    if (invalidUnicode(codePointAt(input, position - 1))) {
                        parseErrorWithFlush(false);
                        emitToken(characters.REPLACEMENT_CHARACTER);
                        if (isSurrogatePair(input, position - 1)) {
                            position++;
                        }
                    } else if (isSurrogatePair(input, position - 1)) {
                        emitToken(character);
                        emitToken(consumeCharacter(input));
                    } else {
                        emitToken(character);
                    }
            }
        ...
        function bogus_comment(input) {
            ...
            while((consumed = consumeCharacter(input)) !== characters.GREATERTHAN_SIGN && consumed !== characters.EOF) {
                ...
                if (invalidCharacters(codePointAt(input, position - 1))) {
                    error = true;
                }
