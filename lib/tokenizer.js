...
(function(exports) {
    ...
    var mustacheVariableRegExp = /^([A-Z0-9_\.\-])$/i;
    ...
    var tokenType = {
        ...
        UnescapedMustacheVariable: "UnescapedMustacheVariable",
        MustacheSectionOpen: "MustacheSectionOpen",
        MustacheSectionClose: "MustacheSectionClose",
        MustacheSection: "MustacheSection",
        MustacheComment: "MustacheComment",
        MustacheInvertedSection: "MustacheInvertedSection",
        MustachePartial: "MustachePartial",
        MustacheSectionAttribute: "MustacheSectionAttribute",
        MustacheAttributeSection: "$MustacheSections"
    };
    ...
    Parser.prototype.insertMustacheVariable = function(name) {
        this.currentNode().childNodes.push({
            childNodes: [],
            type: tokenType.MustacheVariable,
            name: name
        });
    };

    Parser.prototype.insertUnescapedMustacheVariable = function(name) {
        this.currentNode().childNodes.push({
            childNodes: [],
            type: tokenType.UnescapedMustacheVariable,
            name: name
        });
    };

    Parser.prototype.insertMustachePartial = function(name) {
        var currentNode = this.currentNode();
        var previousNode = currentNode.childNodes[currentNode.childNodes.length - 1],
            indent = "";
        if (previousNode && previousNode.type === tokenType.character) {
            var match = previousNode.text.match(/\n([\s]*)$/);
            if (match) {
                indent = match[1];
            } else if (currentNode.childNodes.length === 1 && (match = previousNode.text.match(/^([\s]*)$/))) {
                indent = match[1];
            }
        ...
        currentNode.childNodes.push({
            childNodes: [],
            type: tokenType.MustachePartial,
            name: name,
            indent: indent
        });
    };

    Parser.prototype.insertMustacheSection = function(name) {
        var currentNode = this.currentNode();
        var section = {
            childNodes: [],
            type: tokenType.MustacheSection,
            tagName: currentNode.tagName,
            namespace: currentNode.namespace,
            name: name
        };

        this.openElements.push(section);
        currentNode.childNodes.push(section);
        return section;
    };

    Parser.prototype.insertInvertedMustacheSection = function(name) {
        var currentNode = this.currentNode();
        var section = {
            childNodes: [],
            type: tokenType.MustacheInvertedSection,
            tagName: currentNode.tagName,
            namespace: currentNode.namespace,
            name: name
        };
    ...
    Parser.prototype.closeMustacheSection = function(name) {
        var currentNode = this.currentNode();
        if (currentNode.type !== tokenType.MustacheSection && currentNode.type !== tokenType.MustacheInvertedSection) {
            throw "Currently open node isn't a mustache section, attempting to close #" + name;
        }

        if (currentNode.name !== name) {
            throw "Currently open mustache section (#" + currentNode.name + ") does not match closing tag (/" + name + ")";
        }

        this.openElements.pop();
    };

    Parser.prototype.insertMustacheComment = function(name) {
        this.currentNode().childNodes.push({
            childNodes: [],
            type: tokenType.MustacheComment,
            name: name
        });
    };
    ...
    Parser.prototype.in_body = function(token) {
        ...
        var startForm = function(token) {
            if (self.formElementPointer !== null) {
                self.parseError();
            } else {
                if (self.hasInButtonScope("p")) {
                    self.closePElement();
                }
                self.formElementPointer = self.insertHTMLElement(token[1], token[2]);
            }
        };
        if (token[0] === tokenType.MustacheVariable) {
            this.insertMustacheVariable(token[1]);
        } else if (token[0] === tokenType.UnescapedMustacheVariable) {
            this.insertUnescapedMustacheVariable(token[1]);
        } else if (token[0] === tokenType.MustacheSectionOpen) {
            this.insertMustacheSection(token[1]);
        } else if (token[0] === tokenType.MustacheSectionClose) {
            this.closeMustacheSection(token[1]);
        } else if (token[0] === tokenType.MustacheComment) {
            this.insertMustacheComment(token[1]);
        } else if (token[0] === tokenType.MustacheInvertedSection) {
            this.insertInvertedMustacheSection(token[1]);
        } else if (token[0] === tokenType.MustachePartial) {
            this.insertMustachePartial(token[1]);
        } else if (this.isCharacterToken(token) && this.characterOneOf(token, characters.NULL)) {
            this.parseError();
        } else if (this.isCharacterToken(token) && this.characterOneOf(token, characters.TAB, characters.LF, characters.FF, characters.CR, characters.SPACE)) {
    ...
    Parser.prototype.in_table = function(token) {
        if (token[0] === tokenType.MustacheVariable || token[0] === tokenType.UnescapedMustacheVariable) {
            throw "Can't place Mustache variables directly inside <table>, <thead>, <tbody>, <tfoot>, <tr>, place them inside <th> or <td>";
        } else if (token[0] === tokenType.MustacheSectionOpen ) {
            if (!(/^(tbody|tfoot|thead)$/).test(this.currentNode().tagName)) {
                throw "Can't open Mustache section directly inside <table>, place it inside <thead>, <tbody> or <tfoot>";
            }
            this.insertMustacheSection(token[1]);
        } else if (token[0] === tokenType.MustacheInvertedSection ) {
            if (!(/^(tbody|tfoot|thead)$/).test(this.currentNode().tagName)) {
                throw "Can't open inverted Mustache section directly inside <table>, place it inside <thead>, <tbody> or <tfoot>";
            }
            this.insertInvertedMustacheSection(token[1]);
        } else if (token[0] === tokenType.MustacheComment) {
            this.insertMustacheComment(token[1]);
            ...
            this.insertMustachePartial(token[1]);
        } else if (this.isCharacterToken(token) && (/^(table|tbody|tfoot|thead|tr)$/).test(this.currentNode().tagName)) {
    ...
    Parser.prototype.inMustacheSection = function() {
        var node = this.currentNode();
        return (node.type === tokenType.MustacheInvertedSection || node.type === tokenType.MustacheSection);
    };

    Parser.prototype.tableAnythingElse = function(token) {
        if (this.inMustacheSection()) {
            this.parseError();
            this.in_body(token);
        } else {
            this.parseError();
    ...
    Parser.prototype.in_table_text = function(token) {
        ...
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.NULL)) {
            ...
            if (this.pendingTableCharacterTokens.some(notSpaceCharacterToken)) {
                this.pendingTableCharacterTokens.forEach(function(arg) {
                    self.tableAnythingElse(arg);
                });
            } else  {
                this.pendingTableCharacterTokens.forEach(function(arg) {
                    self.insertCharacterToken(arg);
                });
            }
            this.mode = this.originalMode;
            this.addToken(token);
    ...
    function Tokenizer(options, parser) {
        ...
        this.mustacheData = null;
        this.mustacheState = null;
        this.tokens = [];
    ...
    Tokenizer.prototype.parse = function(input) {
        ...
        switch(this.state) {
            ...
            case this.states.before_attribute_name:
                ...
                break;
            case this.states.mustache_attribute_name:
                this.mustache_attribute_name(input);
                break;
            ...
            case this.states.CDATA_section:
                ...
                break;
            case this.states.mustache_open:
                this.mustache_open(this.consumeCharacter(input));
                break;
            case this.states.mustache_open_open:
                this.mustache_open_open(input);
                break;
            case this.states.mustache_open_open_open:
                this.mustache_open_open_open(input);
                break;
            case this.states.mustache_section_open:
                this.mustache_section_open(input);
                break;
            case this.states.mustache_section_close:
                this.mustache_section_close(input);
                break;
            case this.states.mustache_inverted_section_open:
                this.mustache_inverted_section_open(input);
                break;
            case this.states.mustache_partial:
                this.mustache_partial(input);
                break;
            default:
    ...
    Tokenizer.prototype.currentAttributeSection = function() {
        return this.openAttributeSections[this.openAttributeSections.length - 1];
    };

    Tokenizer.prototype.createAttributeSection = function(sectionName, type) {
        var section = {
            name: sectionName,
            type: type,
            childNodes: []
        };
        if (this.openAttributeSections.length) {
            this.currentAttributeSection().childNodes.push(section);
        } else {
            if (!this.attributes[tokenType.MustacheAttributeSection]) {
                this.attributes[tokenType.MustacheAttributeSection] = [];
            }
            this.attributes[tokenType.MustacheAttributeSection].push(section);
        }
        this.openAttributeSections.push(section);
    };
    ...
    Tokenizer.prototype.addAttribute = function() {
        if (this.attrName && this.attrName.length) {
            if (!this.attributes || this.attributes[this.attrName] !== undefined || this.endTag) {
                ...
                if (this.openAttributeSections.length) {
                    this.currentAttributeSection().childNodes.push({
                        name: this.attrName,
                        value: this.attrValue,
                        type: tokenType.MustacheSectionAttribute
                    });
                } else {
    ...
    Tokenizer.prototype.emitTag = function() {
        if (this.openAttributeSections.length) {
            throw "Unclosed Mustache section in element " + this.tagName;
        }
        this.addAttribute();
        if (!this.endTag) {
    ...
    Tokenizer.prototype.emitMustacheVariable = function(name) {
        this.tokens.push([tokenType.MustacheVariable, name]);
    };

    Tokenizer.prototype.emitUnescapedMustacheVariable = function(name) {
        this.tokens.push([tokenType.UnescapedMustacheVariable, name]);
    };

    Tokenizer.prototype.emitMustacheOpenSection = function(name) {
        this.tokens.push([tokenType.MustacheSectionOpen, name]);
    };

    Tokenizer.prototype.emitMustacheInvertedSection = function(name) {
        this.tokens.push([tokenType.MustacheInvertedSection, name]);
    };

    Tokenizer.prototype.emitMustacheCloseSection = function(name) {
        this.tokens.push([tokenType.MustacheSectionClose, name]);
    };

    Tokenizer.prototype.emitMustacheComment = function(name) {
        this.tokens.push([tokenType.MustacheComment, name]);
    };

    Tokenizer.prototype.emitMustachePartial = function(name) {
        this.tokens.push([tokenType.MustachePartial, name]);
    };
    ...
    Tokenizer.prototype.data_state = function(character, input) {
        switch(character) {
            ...
            case characters.EOF:
                this.emitEOF();
                break;
            case characters.BRACES_OPEN:
                this.mustacheState = this.states.data;
                this.setState(this.states.mustache_open);
                break;
            default:
    ...
    Tokenizer.prototype.before_attribute_name = function(character) {
        switch(character) {
            ...
            case characters.EOF:
                ...
                break;
            case characters.BRACES_OPEN:
                this.mustacheState = this.states.before_attribute_name;
                this.setState(this.states.mustache_attribute_name);
                break;
    ...
    Tokenizer.prototype.mustache_attribute_name = function(input) {
        var character = this.consumeCharacter(input), consumed = [characters.BRACES_OPEN, character], mustacheData = [], end;
        if (character === characters.BRACES_OPEN) {
            character = this.consumeCharacter(input);
            consumed.push(character);
            switch(character) {
                case characters.GREATERTHAN_SIGN:
                    throw "Mustache partials not permitted in attributes";
                case characters.NUMBER_SIGN:
                    mustacheData = this.mustache_consume_name(input, consumed);
                    end = consumed.slice(-2);
                    if (end[0] === characters.BRACES_CLOSE && end[1] === characters.BRACES_CLOSE) {
                        this.addAttribute();
                        this.createAttributeSection(String.fromCharCode.apply(String, mustacheData), tokenType.MustacheSection);
                        this.setState(this.mustacheState);
                        return;
                    }
                    break;
                case characters.SOLIDUS:
                    ...
                    if (end[0] === characters.BRACES_CLOSE && end[1] === characters.BRACES_CLOSE) {
                        this.addAttribute();
                        this.mustache_close_attribute_section(String.fromCharCode.apply(String, mustacheData));
                        this.setState(this.mustacheState);
                        return;
                    }
                    break;
        ...
        if (this.mustacheState === this.states.before_attribute_name) {
            this.createAttribute();
            this.attrValue = "";
        }
        this.position--;
        this.attrName += String.fromCharCode.apply(String, consumed.slice(0, -1));
        this.setState(this.states.attribute_name);
    };

    Tokenizer.prototype.mustache_close_attribute_section = function(name) {
        if (!this.openAttributeSections.length || this.openAttributeSections.pop().name !== name) {
            throw "Attempting to close attribute section " + name;
        }
    };
    ...
    Tokenizer.prototype.attribute_name = function(character) {
        switch(character) {
            ...
            case characters.SPACE:
                ...
                break;
            case characters.BRACES_OPEN:
                this.mustacheState = this.states.attribute_name;
                this.setState(this.states.mustache_attribute_name);
                break;
            case characters.SOLIDUS:
                this.addAttribute();
            ...
            case 0x0051:case 0x0052:case 0x0053:case 0x0054:case 0x0055:
            case 0x0056:case 0x0057:case 0x0058:case 0x0059:case 0x005A:
                this.appendAttributeName(String.fromCharCode(character + 0x0020));
                break;
            case characters.NULL:
                this.parseError();
                this.appendAttributeName(String.fromCharCode(characters.REPLACEMENT_CHARACTER));
                break;
            case characters.EOF:
                this.parseError();
                this.setState(this.states.data);
            ...
            case characters.LESSTHAN_SIGN:
                ...
                /* falls through */
            default:
                this.appendAttributeName(String.fromCharCode(character));
                break;
        }
    };

    Tokenizer.prototype.appendAttributeName = function(str) {
        if (!this.attrName.length)  {
            throw "Can't make attribute name with conditional Mustache section";
        }
        this.attrName += str;
    };
    ...
    Tokenizer.prototype.attribute_value_unquoted = function(character) {
        switch(character) {
            ...
            default:
                this.attrValue += String.fromCharCode(character);
                var mustacheEndSectionAll = /(\{\{\/\s*[A-Z_\-\.0-9]+\s*\}\})$/i;
                var mustacheEndSectionName = /\{\{\/\s*([A-Z_\-\.0-9]+)\s*\}\}$/i;
                if (mustacheEndSectionAll.test(this.attrValue)) {
                    var name = this.attrValue.match(mustacheEndSectionName)[1];
                    this.attrValue = this.attrValue.substring(0, this.attrValue.length - this.attrValue.match(mustacheEndSectionAll)[1].length);
                    this.addAttribute();
                    this.mustache_close_attribute_section(name);
                    this.setState(this.mustacheState);
                }
                break;
        }
    ...
    Tokenizer.prototype.mustache_open = function(character) {
        switch(character) {
            case characters.BRACES_OPEN:
                this.setState(this.states.mustache_open_open);
                break;
            default:
                this.emitToken(characters.BRACES_OPEN);
                this.position--;
                this.setState(this.mustacheState);
                break;
        }
    ...
    Tokenizer.prototype.mustache_consume_name = function(input, consumed) {
        var mustacheData = [];
        var character = this.consumeCharacter(input);
        consumed.push(character);
        character = this.mustache_consume_whitespace(input, character, consumed);

        while(mustacheVariableRegExp.test(String.fromCharCode(character))) {
            mustacheData.push(character);
            character = this.consumeCharacter(input);
            consumed.push(character);
        }

        this.mustache_consume_whitespace(input, character, consumed);
        consumed.push(this.consumeCharacter(input));
        return mustacheData;
    };

    Tokenizer.prototype.mustache_consume_whitespace = function(input, character, consumed) {
        while([characters.TAB, characters.LF, characters.FF, characters.SPACE].indexOf(character) !== -1) {
            character = this.consumeCharacter(input);
            consumed.push(character);
        }
        return character;
    ...
    Tokenizer.prototype.mustache_open_open = function(input) {
        var character = this.consumeCharacter(input), end = [],
            escape = true,
            consumed = [characters.BRACES_OPEN, characters.BRACES_OPEN, character], mustacheData = [];
        switch(character) {
            case characters.BRACES_OPEN:
                this.setState(this.states.mustache_open_open_open);
                break;
            case characters.NUMBER_SIGN:
                this.setState(this.states.mustache_section_open);
                break;
            case characters.SOLIDUS:
                this.setState(this.states.mustache_section_close);
                break;
            case characters.CIRCUMFLEX:
                this.setState(this.states.mustache_inverted_section_open);
                break;
            case characters.GREATERTHAN_SIGN:
                this.setState(this.states.mustache_partial);
                break;
            case characters.EXCLAMATION_MARK:
                while(end[0] !== characters.BRACES_CLOSE || end[1] !== characters.BRACES_CLOSE) {
                    character = this.consumeCharacter(input);
                    consumed.push(character);
                    end = consumed.slice(-2);
                    if (character === characters.EOF) {
                ...
                this.emitMustacheComment(String.fromCharCode.apply(String, mustacheData.slice(0, -2)));
                this.setState(this.mustacheState);
                break;
            case characters.AMPERSAND:
                escape = false;
            ...
            default:
                ...
                if (end[0] !== characters.BRACES_CLOSE || end[1] !== characters.BRACES_CLOSE) {
                    this.emitTokens(consumed);
                    this.position--;
                } else {
                    this.emitMustacheVariable(String.fromCharCode.apply(String, mustacheData));
                }
    ...
    Tokenizer.prototype.mustache_open_open_open = function(input) {
        var character = this.consumeCharacter(input);
        var mustacheData = [], consumed = [characters.BRACES_OPEN, characters.BRACES_OPEN, characters.BRACES_OPEN, character], end;
        character = this.mustache_consume_whitespace(input, character, consumed);
        ...
        this.mustache_consume_whitespace(input, character, consumed);
        consumed.push(this.consumeCharacter(input), this.consumeCharacter(input));
        end = consumed.slice(-3);

        if (end[0] !== characters.BRACES_CLOSE || end[1] !== characters.BRACES_CLOSE || end[2] !== characters.BRACES_CLOSE) {
            this.emitTokens(consumed);
            this.position--;
        } else {
            this.emitUnescapedMustacheVariable(String.fromCharCode.apply(String, mustacheData));
        }

        this.setState(this.mustacheState);
    };

    Tokenizer.prototype.mustache_section_open = function(input) {
        var character = this.consumeCharacter(input);
        var mustacheData = [], consumed = [characters.BRACES_OPEN, characters.BRACES_OPEN, characters.NUMBER_SIGN, character], end;
        character = this.mustache_consume_whitespace(input, character, consumed);
        ...
        if (end[0] !== characters.BRACES_CLOSE || end[1] !== characters.BRACES_CLOSE) {
            ...
            this.emitMustacheOpenSection(String.fromCharCode.apply(String, mustacheData));
        }

        this.setState(this.mustacheState);
    };

    Tokenizer.prototype.mustache_inverted_section_open = function(input) {
        var character = this.consumeCharacter(input);
        var mustacheData = [], consumed = [characters.BRACES_OPEN, characters.BRACES_OPEN, characters.CIRCUMFLEX, character], end;
        character = this.mustache_consume_whitespace(input, character, consumed);
        ...
        if (end[0] !== characters.BRACES_CLOSE || end[1] !== characters.BRACES_CLOSE) {
            ...
            this.emitMustacheInvertedSection(String.fromCharCode.apply(String, mustacheData));
        }

        this.setState(this.mustacheState);
    };

    Tokenizer.prototype.mustache_partial = function(input) {
        var character = this.consumeCharacter(input);
        var mustacheData = [], consumed = [characters.BRACES_OPEN, characters.BRACES_OPEN, characters.GREATERTHAN_SIGN, character], end;
        character = this.mustache_consume_whitespace(input, character, consumed);
        ...
        if (end[0] !== characters.BRACES_CLOSE || end[1] !== characters.BRACES_CLOSE) {
            ...
            this.emitMustachePartial(String.fromCharCode.apply(String, mustacheData));
        }

        this.setState(this.mustacheState);
    };

    Tokenizer.prototype.mustache_section_close = function(input) {
        var character = this.consumeCharacter(input);
        var mustacheData = [], consumed = [characters.BRACES_OPEN, characters.BRACES_OPEN, characters.SOLIDUS, character], end;
        character = this.mustache_consume_whitespace(input, character, consumed);
        ...
        if (end[0] !== characters.BRACES_CLOSE || end[1] !== characters.BRACES_CLOSE) {
            ...
            this.emitMustacheCloseSection(String.fromCharCode.apply(String, mustacheData));
        }
    ...
    Tokenizer.prototype.states = {
        ...
        CDATA_section: 67,
        mustache_open: 80,
        mustache_open_open: 81,
        mustache_open_open_open: 82,
        mustache_section_open: 83,
        mustache_section_close: 84,
        mustache_inverted_section_open: 85,
        mustache_partial: 86,
        mustache_attribute_name: 87
    };
