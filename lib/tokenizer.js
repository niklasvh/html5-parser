...
(function(exports) {
    ...
    Node.prototype.appendChild = function(child) {
        ...
        /*
        if (len && this.childNodes[len - 1].nodeType === 3) {
            token.element.childNodes[len - 1].data += chr;
        } else {
            token.element.appendChild(document.createTextNode(chr));
        }           */
    ...
    Constructor.prototype.appendText = function(node, chr) {
        ...
        if (len && node.childNodes[len - 1].nodeType === tokenType.character) {
            node.childNodes[len - 1].text += chr;
        } else {
            node.appendChild({
                nodeType: tokenType.character,
                text: chr
            });
        }
    };
    ...
    Parser.prototype.addToken = function(token) {
        ...
        if (this.options.type === "tree" && token !== tokenType.parseError) {
            ...
            if (!adjustedCurrentNode ||
                ...
                (adjustedCurrentNode.tagName === "annotation-xml" && adjustedCurrentNode.namespaceURI === namespace.MathML && this.isStartTag(token, "svg")) ||
    ...
    Parser.prototype.pushActiveFormatting = function(element) {
        ...
        while((item = this.activeFormattingElements[markerIndex])) {
            if (element.tagName === item.tagName && element.namespaceURI === item.namespaceURI && this.equalObjects(item.attributes, element.attributes)) {
                count++;
                if (first === null) {
                    first = markerIndex;
                }
    ...
    Parser.prototype.insertCharacter = function(chr) {
        ...
        if (this.fosterParented && this.tokenizer.state !== this.tokenizer.states.plaintext) {
            ...
            if (len && fosterData.index && fosterData.foster.childNodes[fosterData.index - 1].nodeType === tokenType.character) {
    ...
    Parser.prototype.in_foreign_content = function(token) {
        ...
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.NULL)) {
            ...
            // Adjust foreign attributes for the token. (This fixes the use of namespaced attributes, in particular XLink in SVG.)
            this.insertForeignElement(tagName, attributes, this.currentNode().namespaceURI);
        ...
        } else if (this.isEndTag(token, "script") && this.currentNode().tagName === "script" && this.currentNode().namespaceURI === namespace.SVG) {
            this.openElements.pop();
            /*
    ...
    Parser.prototype.inSpecialCategory = function(node) {
        return ((["address", "applet", "area", "article", "aside", "base", "basefont", "bgsound", "blockquote", "body", "br", "button", "caption",
            ...
            "summary", "table", "tbody", "td", "template", "textarea", "tfoot", "th", "thead", "title", "tr", "track", "ul", "wbr", "xmp"].indexOf(node.tagName) !== -1 && node.namespaceURI === namespace.HTML) ||
            (["mi", "mo", "mn", "ms", "mtext", "annotation-xml"].indexOf(node.tagName) !== -1 && node.namespaceURI === namespace.MathML) ||
            (["foreignObject", "desc", "title"].indexOf(node.tagName) !== -1 && node.namespaceURI === namespace.SVG));
    };
    ...
    Parser.prototype.hasInScope = function(target, list, negate) {
        ...
        var matchFunc = function(group) {
            return (group.namespace === node.namespaceURI && group.elements.indexOf(node.tagName) !== -1);
        };
        while(true) {
            if (node === undefined) {
                break;
