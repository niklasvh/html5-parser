(function(exports) {
  ...
  Tokenizer.prototype.curly_open = function(input) {
    var character = this.consumeCharacter(input), end = [],
      consumed = [characters.CURLY_OPEN, character], data = [];

    if (character !== characters.CURLY_OPEN) {
      this.emitToken(characters.CURLY_OPEN);
      this.position--;
    ...
    do {
      ...
      if (character === characters.CURLY_CLOSE) {
        ...
        if (character = this.consumeCharacter(input) !== characters.CURLY_CLOSE) {
          this.emitTokens(consumed);
          this.position--;
          this.setState(this.extensionState);
          return;
        } else {
          this.tokens.push([tokenType.variable, String.fromCharCode.apply(String, data)]);
          this.setState(this.extensionState);
          return;
        }
      } else {
