(function(exports) {
  ...
  Tokenizer.prototype.parse = function(input) {
    ...
    switch(this.state) {
      ...
      case this.states.CDATA_section:
        ...
        break;
      case this.states.curly_open:
        this.curly_open(input);
        break;
      default:
  ...
  Tokenizer.prototype.data_state = function(character, input) {
    switch(character) {
      ...
      case characters.EOF:
        this.emitEOF();
        break;
      case characters.CURLY_OPEN:
        this.extensionState = this.states.data;
        this.setState(this.states.curly_open);
        break;
      default:
  ...
  Tokenizer.prototype.revert_curly = function(consumed) {
    this.emitTokens(consumed);
    this.setState(this.extensionState);
  };

  Tokenizer.prototype.curly_open = function(input) {
    var character = this.consumeCharacter(input),
      consumed = [characters.CURLY_OPEN], name, type;

    if (character !== characters.CURLY_OPEN) {
      this.position--;
      this.revert_curly(consumed);
      return;
    }

    consumed.push(character);

    this.consume_whitespace(input, consumed);
    character = this.consumeCharacter(input);
    if (character === characters.NUMBER_SIGN) {
      type = this.consume_regexp(input, consumed, this.validSectionType);
      this.consume_whitespace(input, consumed);
      name = this.consume_regexp(input, consumed, this.validVariableName);

      if (type.length > 0 && this.attemptClose(input, consumed)) {
        if (name.length > 0) {
          this.tokens.push([tokenType.withBlock, String.fromCharCode.apply(String, type), String.fromCharCode.apply(String, name)]);
        } else {
          this.tokens.push([tokenType.block, String.fromCharCode.apply(String, type)]);
        }
        this.setState(this.extensionState);
      } else {
        this.revert_curly(consumed);
      ...
      name = this.consume_regexp(input, consumed, this.validVariableName);
      if (name.length > 0 && this.attemptClose(input, consumed)) {
        this.tokens.push([tokenType.closeBlock, String.fromCharCode.apply(String, name)]);
        this.setState(this.extensionState);
      } else {
        this.revert_curly(consumed);
      }
      ...
      this.consume_whitespace(input, consumed);
      if (name.length > 0 && this.attemptClose(input, consumed)) {
        this.tokens.push([tokenType.variable, String.fromCharCode.apply(String, name)]);
        this.setState(this.extensionState);
      } else {
        this.revert_curly(consumed);
      }
  ...
  Tokenizer.prototype.attemptClose = function(input, consumed) {
    var character = this.consumeCharacter(input);
    if (character === characters.CURLY_CLOSE) {
      consumed.push(character);
      character = this.consumeCharacter(input);
      if (character === characters.CURLY_CLOSE) {
        return true;
  ...
  Tokenizer.prototype.consume_whitespace = function(input, consumed) {
    var character;
    while([characters.TAB, characters.LF, characters.FF, characters.SPACE].indexOf(character = this.consumeCharacter(input)) !== -1) {
      consumed.push(character);
    }
    this.position--;
  };

  Tokenizer.prototype.validSectionType = (/^([A-Z0-9_\.\-])$/i);
  Tokenizer.prototype.validVariableName = (/^([A-Z0-9_\.\-])$/i);

  Tokenizer.prototype.consume_regexp = function(input, consumed, regexp) {
    var name = [], character;

    while(regexp.test(String.fromCharCode(character = this.consumeCharacter(input)))) {
      name.push(character);
      consumed.push(character);
    }
