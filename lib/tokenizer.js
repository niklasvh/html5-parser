...
(function(exports) {
    ...
    exports.parser = function(html, options) {
        options = options || {};
    ...
    Tokenizer.prototype.flushCharacters = function() {
        if (this.characterBuffer.length > 0) {
            this.tokens.push(['Character', this.characterBuffer]);
            this.characterBuffer = "";
        }
    };

    Tokenizer.prototype.parseError = function() {
        this.parseErrorWithFlush(true);
    };

    Tokenizer.prototype.parseErrorWithFlush = function(flush) {
        if (flush) {
    ...
    Tokenizer.prototype.emitTag = function() {
        ...
        if (!this.endTag) {
            ...
            if (this.selfClosing) {
                this.tokens.push(['StartTag', this.tagName, this.attributes, true]);
            } else {
                this.tokens.push(['StartTag', this.tagName, this.attributes]);
            }
        } else {
            if (this.selfClosing) {
                this.parseError();
            }
            this.tokens.push(['EndTag', this.tagName]);
        }
    };

    Tokenizer.prototype.emitToken = function(chr) {
        this.characterBuffer += String.fromCharCode(chr);
    };

    Tokenizer.prototype.emitTokens = function(chrs) {
        this.characterBuffer += String.fromCharCode.apply(String, chrs);
    };
        ...
        switch(character) {
            ...
            default:
                if (invalidCharacters(character)) {
                    this.parseErrorWithFlush(this.characterBuffer.length === 0);
                } else if(invalidCharacters(this.codePointAt(input, this.position - 1))) {
                    this.parseErrorWithFlush(this.characterBuffer.length === 0);
                }

                if (invalidUnicode(this.codePointAt(input, this.position - 1))) {
                    this.parseErrorWithFlush(false);
    ...
    Tokenizer.prototype.bogus_comment = function(input) {
        ...
        while((consumed = this.consumeCharacter(input)) !== characters.GREATERTHAN_SIGN && consumed !== characters.EOF) {
            ...
            if (invalidCharacters(this.codePointAt(input, this.position - 1))) {
                error = true;
            }
