...
(function(exports) {
    ...
    Parser.prototype.addToken = function(token) {
        if (this.options.type === "tree") {
            switch(this.mode) {
                case this.modes.initial:
                    this.initial(token);
                    break;
                case this.modes.before_html:
                    this.before_html(token);
                    break;
                case this.modes.before_head:
                    this.before_head(token);
                    break;
                case this.modes.in_head:
                    this.in_head(token);
                    break;
                case this.modes.after_head:
                    this.after_head(token);
                    break;
                case this.modes.in_body:
                    this.in_body(token);
                    break;
    ...
    Parser.prototype.insertHTMLElement = function(type) {
    ...
    Parser.prototype.before_head = function(token) {
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.TAB, characters.LF, characters.FF, characters.CR, characters.SPACE)) {
            ...
            this.headElementPointer = this.insertHTMLElement(token[1]);
    ...
    Parser.prototype.in_head = function(token) {
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.TAB, characters.LF, characters.FF, characters.CR, characters.SPACE)) {
            ...
            this.insertHTMLElement(token[1]);
        ...
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.NULL)) {
           ...
           // Act as if an end tag with tag name "body" had been seen, then, if that token wasn't ignored, reprocess the current token.
            ...
            if (this.formElementPointer !== null) {
                ...
                // If the stack of open elements has a p element in button scope, then act as if an end tag with the tag name "p" had been seen.
                this.formElementPointer = this.insertHTMLElement(token[1]);
            }
