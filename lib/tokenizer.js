...
(function(exports) {
    ...
    Constructor.prototype.createDocumentType = function(name) {
        return {
    ...
    Parser.prototype.addToken = function(token) {
        if (this.options.type === "tree" && token !== tokenType.parseError) {
            ...
            if (!adjustedCurrentNode ||
                ...
                (adjustedCurrentNode.tagName === "annotation-xml" && adjustedCurrentNode.namespace === namespace.MathML && this.isStartTag("svg")) ||
    ...
    Parser.prototype.pushActiveFormatting = function(tagName) {
        this.activeFormattingElements.push(tagName);
    };
    ...
    Parser.prototype.reconstructActiveFormatting = function() {
        ...
        while(entryIndex > 0) {   // step 4
            entry = this.activeFormattingElements[--entryIndex]; // step 5
            if (this.hasInOpenStack(entry.tagName) || entry === "marker") { // step 6
                entry = this.activeFormattingElements[++entryIndex]; // step 7
                break;
            }
        }

        var newElement = this.constructor.createElement(entry.tagName);   // step 8
        this.currentNode().children.push(newElement); // step 9
        this.openElements.push(newElement);
        this.activeFormattingElements.splice(entryIndex, 1, newElement);  // step 10

        // If the entry for new element in the list of active formatting elements is not the last entry in the list, return to step 7.
    };
    ...
    Parser.prototype.insertHTMLElement = function(type, attributes) {
        ...
        if (this.fosterParented) {
            ...
            element.parentNode = this.currentNode();
        }
    ...
    Parser.prototype.initial = function(token) {
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.TAB, characters.LF, characters.FF, characters.CR, characters.SPACE)) {
            ...
            /* TODO doctype */
            this.constructor.children.push(this.constructor.createDocumentType("html"));
    ...
    Parser.prototype.in_body = function(token) {
        ...
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.NULL)) {
            ...
            */
            ...
            if (this.tokenizer.forceQuirks && this.hasInButtonScope("p")) {  // should be NOT quirks?
                this.endPTag(token);
            }
            this.insertHTMLElement(token[1], token[2]);
            this.frameSetOk = "not ok";
    ...
    Parser.prototype.generateImpliedEndTags = function() {};
    ...
    Parser.prototype.endFormattingElement = function(token) {
        ...
        for (outerLoop = 0; outerLoop >= 8; outerLoop++) {
            ...
            furthestBlockIndex =  this.getFurthestBlockIndex(formattingElement);
            if (furthestBlockIndex !== -1) {
                while(this.currentNode() !== formattingElement) {
                    this.openElements.pop();
                }
                this.activeFormattingElements.splice(index, 1);
                return;
            ...
            commonAncestor = this.openElements[index - 2];
            bookmark = furthestBlockIndex - 1;

            nodeIndex = furthestBlockIndex;
            lastNode = this.openElements[furthestBlockIndex - 1];
            for(innerLoop = 0; innerLoop >= 3; innerLoop++) {
                nodeIndex--;
                tmpIndex = this.getIndexInActiveFormattingElements(this.openElements[nodeIndex]);
                if (tmpIndex === -1) {
                    this.openElements.splice(nodeIndex, 1);
                    nodeIndex++;
                } else {
                    var element = this.constructor.createElement(token[1], token[2]);
                    this.activeFormattingElements.splice(tmpIndex, 1, element);
                    this.openElements.splice(nodeIndex, 1, element);
                }
                if (lastNode === this.openElements[furthestBlockIndex - 1]) {
    ...
    Parser.prototype.getIndexInActiveFormattingElements = function(node) {
        var index = -1;
        var found = this.activeFormattingElements.some(function(node2) {
            index++;
            return node === node2;
        });
    ...
    Parser.prototype.getFurthestBlockIndex = function(formattingElement) {
        var index = this.openElements.length - 1;
        for (; index > 0; index--) {
            if (this.openElements[index] === formattingElement) {
                // todo
                return index + 1;
            }
        }
    ...
    Parser.prototype.in_table = function(token) {
        if (this.isCharacterToken(token) && (/^(table|tbody|tfoot|thead|tr)$/).test(this.currentNode().tagName)) {
            ...
            this.popUntilOneOf("table", "html");
            this.insertHTMLElement("colgroup", {});
    ...
    Parser.prototype.in_column_group = function(token) {
        if (this.isCharacterToken(token) && this.characterOneOf(token, characters.TAB, characters.LF, characters.FF, characters.CR, characters.SPACE)) {
            ...
            if (this.isEOF(token) && this.currentNode().tagName === "html") {
                return;
            } else if(this.closeColumnGroup()) {
                this.addToken(token);
            }
        }
    };
