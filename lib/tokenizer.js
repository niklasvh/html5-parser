...
(function(exports) {
    ...
    Parser.prototype.closeMustacheSection = function(name) {
        ...
        if (currentNode.type !== tokenType.MustacheSection && currentNode.type !== tokenType.MustacheInvertedSection) {
            throw new Error("Currently open node isn't a mustache section, attempting to close #" + name);
        }

        if (currentNode.name !== name) {
            throw new Error("Currently open mustache section (#" + currentNode.name + ") does not match closing tag (/" + name + ")");
        }
    ...
    Parser.prototype.in_body = function(token) {
        ...
        if (this.isMustacheVariable(token)) {
            this.insertMustacheVariable(token[1]);
        } else if (this.isUnescapedMustacheVariable(token)) {
    ...
    Parser.prototype.isMustacheVariable = function(token) {
        return token[0] === tokenType.MustacheVariable;
    };

    Parser.prototype.isUnescapedMustacheVariable = function(token) {
        return token[0] === tokenType.UnescapedMustacheVariable;
    };
